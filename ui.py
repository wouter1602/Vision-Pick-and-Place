...
class Ui:
    def __init__(self, s: Settings):
        ...
        button_view = [
            [sg.Radio('Auto detect', "Radio", default=s.autodetect),
             sg.Radio('Manual detect', "Radio", default=(not s.autodetect))],
            [sg.Button('Exit', size=(10, 1)),
             sg.Button('Detect', size=(10, 1))]
        ]

        self.layout = self._begin_layout + detection_view + button_view
    ...
    def update(self, s: Settings, v: Video) -> int:
        ...
        if self._event == sg.WIN_CLOSED or self._event == 'Exit':
            return 1
        elif self._event == 'Detect':
            self.update_detect = True

        if self.update_layout(s):
            return 2
    ...
    def _update_img(self, s: Settings, v: Video):
        if s.edge_detection_type == 1:
            imgbytes_view_3 = cv.imencode('.png', v.thrash)[1].tobytes()
        elif s.edge_detection_type == 2:
            imgbytes_view_3 = cv.imencode('.png', v.canny)[1].tobytes()
        else:
            imgbytes_view_3 = cv.imencode('.png', v.canny)[1].tobytes()

        imgbytes_view_4 = cv.imencode('.png', v.video_output)[1].tobytes()
